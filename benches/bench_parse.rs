...
pub fn bench_parse(c: &mut Criterion) {
    ...
    for _i in 0..BATCH_SIZE {
        ...
        write!(input_with_offset, "{}T{}{}{:02}:00", ndt.date(), ndt.time(), offset_sign, offset.abs()).unwrap();
    ...
    c.benchmark_group("parse_offset")
        .throughput(Throughput::Bytes((input_with_offset.len() + BATCH_SIZE * std::mem::size_of::<i64>()) as u64))
        .bench_function("parse_scalar", |b| {
            b.iter(|| bench_parse_scalar(input_with_offset.as_bytes(), &mut output, DATE_LEN_WITH_OFFSET))
        })
        .bench_function("parse_simd", |b| {
            b.iter(|| bench_parse_simd(input_with_offset.as_bytes(), &mut output, DATE_LEN_WITH_OFFSET))
        })
        .bench_function("parse_chrono", |b| {
            b.iter(|| bench_parse_chrono(input_with_offset.as_bytes(), &mut output, DATE_LEN_WITH_OFFSET))
        });
}
